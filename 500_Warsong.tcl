...
namespace eval Warsong {
	...
	variable FlagCarriers [list 0 0]
	variable Scores [list 0 0]
	variable Spectators ""
	variable GotReward ""
		...
		for { set num 1 } { $num <= $::Warsong::QueueCount } { incr num } {
			foreach side {0 1} {
				...
				foreach player $::Warsong::Queue($num,$side) {
					if { [lsearch $args $player]>=0 || $args=="*" } { set removed 1
...
# 		}
	...
	proc ClearScores {} {
		variable Scores; set Scores [list 0 0]
	}
	...
	proc IsInTeam { player } {
		variable Team
		expr {[lsearch [concat $Team(0) $Team(1)] $player] >= 0}
	}
	...
	proc StartGame {} {
		...
		set ::Warsong::FlagCarriers [list 0 0]
...
# 		}
	...
	proc RemoveFromTeam { player {no_teleport 0} } {
		...
		set side [::Custom::GetPlayerSide $player]
		set newteam ""
		foreach p $Team($side) { if { $p != $player } { lappend newteam $p } }
		set Team($side) $newteam
	...
	proc Commands { player cargs } {
		if { $cargs == "" } {
			...
			switch -- [lindex $cargs 0] {
				...
				"spec" {
					if { [GetPlevel $player] < 2 } { return [::Texts::Get "not_allowed_command"] }
					if { [lsearch $Spectators $player] < 0 } {
						append msg [::Texts::Get "joined_spectators"]
						lappend Spectators $player
					} else {
						append msg [::Texts::Get "left_spectators"]
						set new_specs ""
						foreach spec $Spectators { if { $spec != $player } { lappend new_specs $spec } }
						set Spectators $new_specs
					}
					RunChecks
					return $msg
				}
				...
				"reload" {
					...
					if { !$::Warsong::USE_CONF_FILE } { return [::Texts::Get "no_config"] }
	...
	namespace eval Vendor {
		proc GossipHello { npc player } {
			set option0 "text 6 \"[::Texts::GetEx [::Texts::Locale] ::Warsong browse_goods]\""
			set option1 "text 0 \"[::Texts::GetEx [::Texts::Locale] ::Warsong reputation_info]\""
			...
			switch -- $option {
				0 {
					if { $repu >= $reqrepu } {
						...
						set reqrepuname [lindex [::Texts::GetEx [::Texts::Locale] ::Warsong reputation_levels] [expr {$reqrepu+3}]]
						::SendGossip $player $npc "text 6 \"[::Texts::GetEx [::Texts::Locale] ::Warsong cant_buy $reqrepuname]\""
					}
				}
				1 {
					set repuname [lindex [::Texts::GetEx [::Texts::Locale] ::Warsong reputation_levels] [expr {$repu+3}]]
					::SendGossip $player $npc "text 0 \"[::Texts::GetEx [::Texts::Locale] ::Warsong your_reputation $repuname]\""
				}
			}
		}
	...
	namespace eval Quest {
			...
			foreach qs $quest_status {
				if { $qs == 3 } {
					if { [Warsong::IsInQueue $player] } {
						set gossip "text 2 \"[::Texts::GetEx [::Texts::Locale] ::Warsong warsong_remove]\""
					} else {
						set gossip "text 2 \"[::Texts::GetEx [::Texts::Locale] ::Warsong warsong_play]\""
					}
		...
		proc GossipSelect { npc player option } {
			if { [::Warsong::AddToQueue $player] } {
				SendGossip $player $npc "text 0 \"[::Texts::GetEx [::Texts::Locale] ::Warsong joined_queue]\""
			} elseif { [::Warsong::RemoveFromQueue $player] } {
				SendGossip $player $npc "text 0 \"[::Texts::GetEx [::Texts::Locale] ::Warsong left_queue]\""
			}
		}
	...
	namespace eval Battle {
		proc GossipHello { npc player } {
			if { [Warsong::IsInQueue $player] } {
				set option0 "text 2 \"[::Texts::GetEx [::Texts::Locale] ::Warsong warsong_remove]\""
			} else {
				set option0 "text 2 \"[::Texts::GetEx [::Texts::Locale] ::Warsong warsong_play]\""
			}
		...
		proc GossipSelect { npc player option } {
			if { [::Honor::CheckAddOnVersion $player] } {
				if { [::Warsong::RemoveFromQueue $player] } {
					::SendGossip $player $npc "text 0 \"[::Texts::GetEx [::Texts::Locale] ::Warsong warsong_unqueued]\""
				} else {
					if { [::Warsong::IsOpen] && [::Warsong::AddToTeam $player] } {
					} elseif { [::Warsong::AddToQueue $player] } {
						::SendGossip $player $npc "text 0 \"[::Texts::GetEx [::Texts::Locale] ::Warsong warsong_queued]\""
					} else {
						::SendGossip $player $npc "text 0 \"[::Texts::GetEx [::Texts::Locale] ::Warsong warsong_error]\""
					}
				}
			}
		}
	...
	namespace eval Spirit {
		proc GossipHello { npc player } {
			SendGossip $player $npc "text 0 \"[::Texts::GetEx [::Texts::Locale] ::Warsong \"Hello?\"]\""
		}
		proc GossipSelect { npc player option } {
		}
		proc QuestStatus { npc player } {
