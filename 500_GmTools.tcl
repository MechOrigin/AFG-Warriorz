...
# Name:		GmTools.tcl
#
# Description:	GM utility commands based on dbconsole.exe
#
	...
	foreach tool { "ngconsole.exe" "dbconsole.exe" } {
		foreach dir { "." "bin" $::StartTCL::basedir $::StartTCL::SCRIPTS_DIR } {
			if { [ file exists "$dir/$tool" ] } {
				variable dbconsole "$dir/$tool"
				break
			}
		}
	}

	if { ! [ info exists dbconsole ] } {
...
#
#	proc ::GmTools::_dbconsole { command }
#
proc ::GmTools::_dbconsole { command } {
	variable dbconsole
	set r ""
	set fp [ open "| $dbconsole $command" ]
...
proc ::GmTools::gtGetAccountInfo { account } {
	set r 0
	set status ""
	set data [ _dbconsole "find $account" ]
...
proc ::GmTools::gtSetPassword { account password } {
	set r 0; set status ""
	set data [ _dbconsole "password $account $password" ]
	regexp -nocase -- {<status>(.*?)</status>} $data gr status
...
proc ::GmTools::gtAddAccount { account password } {
	set r 0
	set status ""
	set data [ _dbconsole "add $account $password" ]
	regexp -nocase -- {<status>(.*?)</status>} $data gr status
...
proc ::GmTools::gtDelAccount { account } {
	set r 0
	set status ""
	set data [ _dbconsole "del $account" ]
	regexp -nocase -- {<status>(.*?)</status>} $data gr status
...
proc ::GmTools::gtSetPlevel { account plevel } {
	set r 0
	set status ""
	set data [ _dbconsole "plevel $account $plevel" ]
	regexp -nocase -- {<status>(.*?)</status>} $data gr status
...
proc ::GmTools::gtGetAccount { guid } {
	...
	set name 0
	set data [ _dbconsole "getacc $guid" ]
	regexp -nocase -- {<name>(.*?)</name>} $data gr name
...
proc ::GmTools::gtBan { account } {
	set r 0
	set status ""
	set data [ _dbconsole "ban $account" ]
	regexp -nocase -- {<status>(.*?)</status>} $data gr status
...
proc ::GmTools::gtUnBan { account } {
	set r 0
	set status ""
	set data [ _dbconsole "unban $account" ]
	regexp -nocase -- {<status>(.*?)</status>} $data gr status
...
proc ::GmTools::gtLock { account } {
	set r 0
	set status ""
	set data [ _dbconsole "lock $account" ]
	regexp -nocase -- {<status>(.*?)</status>} $data gr status
...
proc ::GmTools::gtUnLock { account } {
	set r 0
	set status ""
	set data [ _dbconsole "unlock $account" ]
	regexp -nocase -- {<status>(.*?)</status>} $data gr status
...
proc ::GmTools::cAccounts { player cargs } {
	...
	eval "::SendGossip ${player} ${player} \{ text 0 \"$header\" \} ${ml}"
}
...
proc ::GmTools::cPass { player cargs } {
	if { [ string length $cargs ] } {
		set account [ ::GmTools::gtGetAccount [ ::GetGuid $player ] ]
	        set password [ lindex [ split $cargs ] 0 ]
...
proc ::GmTools::cChPass { player cargs } {
	...
	if { [ llength $cargs ] } {
		...
		set r [::GmTools::gtSetPassword [lindex $account 1] [lindex $cargs 1]]
...
proc ::GmTools::cAccInfo { player cargs } {
	...
	if { $account != 0 } {
		...
		eval "::SendGossip \$player \$player \{ text 0 \"Information for account $name\" \}\
...
#
#	proc ::GmTools::cTcl { player cargs }
#
proc ::GmTools::cTcl { player cargs } {
	set ::p $player
...
proc ::GmTools::cUntalent { player cargs } {
	...
	if { ! [ ::ChangeMoney $target -$cost ] } {
		return [ ::Texts::Get "not_enough_money"[ ::GetName $target ] $cost ]
	}

	::LearnSpell $target $spellid
	return [ ::Commands::setcp $points ]
}


#
# logging of commands (this must be called both after the ::Custom::AddCommand
