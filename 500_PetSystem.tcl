...
namespace eval stbmstpets {
	...
	close $id
	set pet1 [lindex $list 0]
	set pet2 [lindex $list 1]
	set pet3 [lindex $list 2]
	set pet 0
		if { $pet1 != 0 } {
			set pet $pet1
			set pet1 0
	...
	proc dismiss { player } {
		...
		set petguid [GetLinkObject $pet]
			if { $player != $petguid } {
		return "You must select your pet first!"
	}
	set pethp [GetHealthPCT $pet]
		if { $pethp < 90 } {
	return "Your pet needs more life!"
	}
	set pclass [GetClass $player]
		if { $pclass != 9 && $pclass!= 3 } { return }
		if { ![file exists "extra/pets/$pname"] } {
	set file "extra/pets/$pname"
	set id [open $file w+]
	puts $id "0 0 0"
	close $id
