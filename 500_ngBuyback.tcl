...
namespace eval ::ngBuyback {
	...
	variable l_hlp02 "NextGen Buyback System (v%2\$s) - Usage:%1\$c%1\$c%3\$s \[ %4\$s \| %5\$s \| %6\$s \]%1\$c%4\$-12s - resets the buyback data%1\$c%5\$-13s - displays the current program version%1\$c%6\$-15s - shows this info"
...
#
# "buyback_help" -
# "help_buyback" { return [ ::ngBuyback::help $player ] }
#
# inside:
#
...
proc ::ngBuyback::help { player } {
	...
	if { [ GetPlevel $player ] < $::ngBuyback::gm_level } {
		...
		return [ format $::ngBuyback::l_hlp02 10 $VERSION ".buyback" "reset" "version" "help" ]
	}
}
...
#
# "buyback_version" -
# "version_buyback" { return [ ::ngBuyback::version ] }
#
# inside:
#
# switch [string tolower $command]
...
proc ::ngBuyback::sell { player iteminfo } {
	...
	if { [ ::GetScpValue "items.scp" "item $item_id" "bonding" ] == 4 } {
...
proc ::ngBuyback::rebuy { player slot } {
	...
	if { ! $slot || ! [ info exists ::ngBuyback::BuybackDataArray${player}($slot) ] } {
		return $ERR_BUYBACK_OK
