...
namespace eval Honor {
	...
	#variable HasLoginScriptEffect 0
	...
	proc GetAddOnVersion { player } {
		...
		set pn [string tolower $player_name]
		ReadData $player_name
		if { [clock seconds] - [lindex $HonorData($pn) 9] > 86400 } { return 0 }
	...
	proc ChangeRating { player {points 0} {kill_type 0} {comment ""} } {
		...
		# to log player position, add [GetPos $player] as a parameter to ChangeRatingByName
		if { [ChangeRatingByName $player_name $points $kill_type $comment] } {
	...
	proc ChangeRatingByName { player_name {points 0} {kill_type 0} {comment ""} {pos ""} } {
		variable HonorData
		set pn [string tolower $player_name]
		ReadData $player_name
		...
		if { $player_honor > 65000 } { set player_honor 65000 }
		switch -- $kill_type {
		...
		if { [WriteData $player_name] } {
			ClearNames
			if { $comment != "" } { Log $player_name $player_honor $points $comment $pos }
			return 1
		}
		return 0
	}

	# check if the player's place is allowed for honor
	...
	proc Log { name rating points msg {pos ""} } {
		...
		if { ![info exists ::Honor::LogHandle] } {
			set ::Honor::LogHandle [open "logs/honor.log" a]
		}
		set log "[::Custom::DateString]: player=$name rating=$rating points=$points event=\[$msg\]"
		if { $pos != "" } { append log " pos=\[[::Custom::RoundPos $pos]\]" }
		puts $::Honor::LogHandle $log
		#close $::Honor::LogHandle
		#unset ::Honor::LogHandle
	}
			...
			if { [info exists $var($hitter)] } {
				...
				if { [::GetObjectType $player] == 4 } {
					if { ![::Custom::IsOnline $player] && [::GetHealthPCT $player] < 100 } {
						if { ![::CastSpell $player $player 7] } { ::CastSpell $hitter $player 5	}
	...
	# to be called when a player is being hitted, in WoWEmu::DamageReduction
	variable CountHitsClean 0
	...
	proc CalcGain { killer victim } {
		if { [::Custom::IsTrivial $victim $killer] } { return 0 }
	...
	proc CalcRatings { victim {killer 0} } {
		...
		# some cleaning
		if { $time - $::Honor::CountHitsClean > 200 } {
			foreach var [info vars ::Honor::Hits::*] {
		...
		if { $time - $::Honor::CountAttackersClean > 300 } {
		...
		if { [::GetObjectType $victim] == 4 } {
			...
			if { [CheckGM $victim] } { return }

			if { $::Honor::PLAYER_RATE } { PlayerKilled $victim $killer }

			if { [info exists ::Honor::Hits::$victim] } { unset ::Honor::Hits::$victim }
			...
			if { [info exists ::Honor::Attacked::$victim] } { unset ::Honor::Attacked::$victim }
	...
	proc PlayerKilled { player {killer 0} } {
		...
		if { $::Honor::CHECK_GHOST && ![::Custom::PlayerIsAlive $player] && $killer } {
			if { ![::CastSpell $killer $killer 7] } { ::CastSpell $killer $killer 5	}
			return
		}
	...
	proc UpdateRankings { {force 0} } {
		...
		foreach name $names {
			...
			set line "<player><pos>$pos</pos><name>$name</name><guild>$guild</guild><level>$level</level><race>$race</race><class>$class</class><kills>$hkills</kills>"
...
# 		if { $time - $CheckOnlineTime > 43200 } {
# 			::Custom::ReadOnline
# 			foreach name $::Custom::OnlinePlayers {
# 				if { $time - [lindex $::Custom::OnlinePlayers($name) 6] > $::Custom::OnlinePlayerUpdate } {
#					SetAddOnVersionByName $name 0
# 				}
# 			}
# 			set CheckOnlineTime $time
# 		}
		...
		if { $cargs == "" } {
			...
			switch -- [lindex $cargs 0] {
				...
				"_frame" {
					if { [llength $cargs] > 1 } {
						# inspect frame
						set player_name [lrange $cargs 1 end]
						set msg "HONOR_INSPECT,[GetUIHonorData $player_name],$player_name"
					} else {
						# honor frame
						set msg [GetNotification $player]
						if { $msg != "" } { return "HONOR_NOTIFY,[string map "{,} {\\,}" $msg]" }
				...
				"_login" {
					if { [info exists ::Honor::LoginScriptEffect($player)] } {
						set time [expr [clock seconds]-$::Honor::LoginScriptEffect($player)]
						if { $time > 10 } {
							CheckLogin $player
							#KickPlayer $player
							return
				...
				"reload" {
					if { [::GetPlevel $player] < $gm_plevel } { return [::Texts::Get "not_allowed_command"] }
					if { !$::Honor::USE_CONF_FILE } { return [::Texts::Get "no_config"] }
				...
				"sql" {
					...
					if { [UsingSQLdb] } {
						...
						switch -- [lindex $cargs 1] {
							...
							"reset" {
								if { [SQLdb_ResetTable] } {
									set ret [::Texts::Get "table_reseted"]
								} else {
									set ret [::Texts::Get "table_not_exists"]
								}
							}
							"query" {
								...
								if { [catch { set ret [join [::SQLdb::querySQLdb $nghandle $sql] "\n"] } err] } {
									...
									switch -- [string tolower [lindex $sql 0]] {
										"delete" { ClearNames; UpdateRankings }
										"insert" -
										"replace" -
										"update" { ClearCache; UpdateRankings }
									}
								}
							}
							default {
				...
				default {
					...
					set help "honor: rankings <$start> <$end>, info <$name/$sel>, del <$name/$sel/*>, change <(+-)$pts> <$name/$sel>, reload, version, sql"
					return $help
				}
			}
		}
	...
	variable CountAttackersClean 0
	...
	# when a racial leader is killed, to be used in WoWEmu::CalcXP
	...
	# check if an NPC is a racial leader
	proc IsLeader { npc } { expr {[lsearch {4949 3057 10181 1748 2784 3516 31462 31459} [::GetEntry $npc]] >= 0} }

	# check if an NPC is a cilivian
	...
	variable GuardsList {68 197 234 261 464 487 489 490 727 821 823 851 853 869 870 874 876 1064 1089 1090\
		...
		12480 12481 12580 13839 14714 14717 31453 31454 31457 31458 31460 31461}
		...
		if { [info exists ::Custom::VERSION] } {
			...
			if { $current_version < $CUSTOM_VERSION } {
				if { [info procs ::Custom::Error] != "" } {
					...
					error "Honor: Error in \"Custom\" script!"
				}
			}
		} else {
			error "Honor: \"Custom\" script is missing!"
		}
	}
...
namespace eval Honor {
	...
	proc SQLdb_Register {} {
		...
		# NAME is the script name (this MUST be consistent accross versions)
		...
		if { ! [ ::SQLdb::booleanSQLdb $nghandle "SELECT * FROM `$::SQLdb::NAME` WHERE (`name` = '$NAME') LIMIT 1" ] } {
			...
			puts "$NAME: Current version ($VERSION) is a new installation."
			...
			if { [ expr { $oldver > $VERSION } ] } {
				...
				puts "$NAME: Current version ($VERSION) is older than the previous one ($oldver)."
				...
				puts "$NAME: Current version ($VERSION) is newer than the previous one ($oldver)."
				::SQLdb::querySQLdb $nghandle "UPDATE `$SQLdb::NAME` SET `version` = '$VERSION', `previous` = '$oldver', `date` = CURRENT_TIMESTAMP WHERE (`name` = '$NAME')"
			}
		}
	}
			...
			while { [catch {set rows [::SQLdb::querySQLdb $nghandle "SELECT `[join $GoodColumns {`, `}]` FROM `honor` LIMIT 1"]} err] } {
				if { [string match -nocase "*column*" $err] } {
					...
					foreach column [lrange $GoodColumns 1 end] {
						if { [string match -nocase "*$column*" $err] } {
							set new_columns ""
							foreach c $GoodColumns { if { $c != $column } { lappend new_columns $c } }
							set GoodColumns $new_columns
	...
	proc SQLdb_RepairTable {} {
		...
		set renamed 0
			...
			foreach row [::SQLdb::querySQLdb $nghandle "SELECT `[join $GoodColumns {`, `}]` FROM `honor` LIMIT 8000"] {
				foreach $GoodColumns $row {
					# correct the data
		...
		if { $err != "" } {
			if { $renamed } { ::SQLdb::querySQLdb $nghandle "ALTER TABLE `$tmpname` RENAME TO `honor`" }
			::Custom::Error $err
	...
	# v4.10 table (for testing/reference)
# 	proc SQLdb_CreateTable410 {} {
# 		variable nghandle
...
# 			`addon_time` INTEGER NOT NULL DEFAULT 0,\
...
# 	}
	...
	proc SQLdb_RedefineProcs {} {
		...
		proc ::Honor::GetRankedPlayerNames {} {
			...
			if { $RankedPlayerNames == "" } {
				variable nghandle
				set RankedPlayerNames [::SQLdb::querySQLdb $nghandle "SELECT `name` FROM `honor` WHERE (`race` != 0) ORDER BY `rating` DESC LIMIT 2000"]
			}
			return $RankedPlayerNames
		}
	...
	proc SQLdb_VersionInfo { {full 1} } {
		if { [info exists ::SQLdb::NAME] } { set name $::SQLdb::NAME } else { set name "SQLdb" }
		if { [info exists ::SQLdb::VERSION] } { set name "$name v$::SQLdb::VERSION" }
		if { [info exists ::SQLdb::s_dbEngine] && $full } {
			set engine $::SQLdb::s_dbEngine
			#if { [info exists ::SQLdb::s_dbEngineVersion] } { set engine "$engine v$::SQLdb::s_dbEngineVersion" }
...
namespace eval Honor {
	...
	if { $::Honor::SELF_INTEGRATION } {
		...
		::Custom::HookProc "WoWEmu::CalcXP" {::Honor::CalcRatings $victim $killer} "Honor::CalcRatings"
		...
		if { $::Honor::CIVILIAN_NOAGGRO && [string first {IsCivilian} [info body "AI::CanAgro"]] < 0 } {
		...
		# improved login detection if spell 836 (StartTCL v0.9.0 or higher needed)
		::Custom::AddSpellScript "::Honor::OnLogin" 836

		::Custom::AddCommand "honor" "::Honor::Commands"
	}
