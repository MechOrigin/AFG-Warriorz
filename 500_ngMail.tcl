...
namespace eval ::ngMail {
	...
	variable l_hlp02 "NextGen Mail System (v%2\$s) - Usage:%1\$c%1\$c%3\$s \[ %4\$s \| %5\$s \| %6\$s \| %7\$s \| %8\$s \]%1\$c%4\$-12s - removes wasted space from the database%1\$c%5\$-15s - recreates the tables from scratch%1\$c%6\$-11s - displays the underlying database API version%1\$c%7\$-13s - displays the current program version%1\$c%8\$-15s - shows this info"
...
proc ::ngMail::help { player } {
	...
	if { [ GetPlevel $player ] < $::ngMail::gm_level } {
		...
		return [ format $::ngMail::l_hlp02 10 $VERSION ".mail" "cleanup" "redo" "dbversion" "version" "help" ]
	}
}
